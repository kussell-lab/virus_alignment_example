...
func readAlignments(done <-chan struct{}, file string) (<-chan Alignment, <-chan error) {
	...
	go func() {
		...
		for {
			...
			if len(alignment) > 0 {
				numAln++
				terms := strings.Split(alignment[0].Id, " ")
				gene := terms[0]
				alnID := strings.Split(gene, "|")[1]
				genepos := terms[1]
				select {
				case alignments <- Alignment{alnID, genepos, alignment}:
				case <-done:
...
func digester(done <-chan struct{}, alignments <-chan Alignment, genes chan<- result, id int, wg *sync.WaitGroup) {
	...
	for aln := range alignments { // HLpaths
		...
		gene := result{aln.ID, aln.pos, aln}
		//writeAln(aln, outdir)
		select {
		//case c <- aln.num:
		case genes <- gene:
...
func writeAln(aln Alignment, outdir string) {
	...
	geneName := aln.ID + "_" + aln.pos
	MSA := filepath.Join(outdir, geneName+".xmfa")
	f, err := os.Create(MSA)
	if err != nil {
		panic(err)
	}
